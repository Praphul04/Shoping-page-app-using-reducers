    ...
    return async (dispatch) => {
        const fetchData = async () => {
            ...
            const response = await fetch(
                'https://react-deployment-demo-346b1-default-rtdb.firebaseio.com/cart.json'
            );
            if(!response.ok) {
...
export const sendCartData = (cart) => {
    return async (dispatch) => {
      ...
      const sendRequest = async() => {
        const response = await fetch('https://react-deployment-demo-346b1-default-rtdb.firebaseio.com/cart.json',
      {
        method: 'PUT',
